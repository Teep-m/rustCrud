...
pub async fn init_db() -> DbClient {
    let db_host = env::var("DATABASE_URL")
        ...
        match Surreal::new::<Http>(&db_host).await {
            ...
            Err(e) => {
                ...
                if retries >= max_retries {
                    panic!("Failed to connect to SurrealDB after {} retries: {}", max_retries, e);
                }
                println!("⚠️  接続失敗 ({}/{}): {}. 再試行します...", retries, max_retries, e);
